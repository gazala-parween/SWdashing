...
class Reddit
    ...
    def getTopPostsPerSubreddit()  
        #puts "hello"
    uri = URI('http://0.0.0.0:8080/api/?user_id=56a638cfc44c289abcd23230')
...
SCHEDULER.every '20s', :first_in => 0 do |job|
  ...
  send_event('reddit56a638cfc44c289abcd23230', { :posts => posts })
end		
...
SCHEDULER.every '20s', :first_in => 0 do |job|
    ...
    for h in handles
      ...
      send_event('twitter_mentions156a638cfc44c289abcd23230', comments: twee_arr[0])
      send_event('twitter_mentions256a638cfc44c289abcd23230', comments: twee_arr[1])
	  send_event('twitter_mentions356a638cfc44c289abcd23230', comments: twee_arr[2])
      send_event('twitter_mentions456a638cfc44c289abcd23230', comments: twee_arr[3])
...
SCHEDULER.every '20s', :first_in => 0 do |job|
  ...
  while max > (min+page_size)
    send_event('news56a638cfc44c289abcd23230', { items: newswhip_article.slice(min, page_size)})
	sleep(10)
  ...
  send_event('news56a638cfc44c289abcd23230', { items: newswhip_article.slice(min..max)})
...
arr = []
 ...
 for i in 0..4
	send_event('youtube_player156a638cfc44c289abcd23230', { :items => channel_arr[0].slice(min, len) })
	send_event('youtube_player256a638cfc44c289abcd23230', { :items => channel_arr[1].slice(min, len) })
	send_event('youtube_player356a638cfc44c289abcd23230', { :items => channel_arr[2].slice(min, len) })
	send_event('youtube_player456a638cfc44c289abcd23230', { :items => channel_arr[3].slice(min, len) })
    sleep(60)
    min=min+1    
 end
