...
SCHEDULER.every '1m', :first_in => 0 do |job|
  ...
  uri = URI('http://0.0.0.0:8080/api/?user_id=56c41d36c44c2834e1d2fbad')
  ...
  if subreddit.length>0
        ...
        for it in subreddit
            ...
            send_event('reddit_56c41d36c44c2834e1d2fbad', { :posts => posts })
        end
    end
end		
...
SCHEDULER.every '20s', :first_in => 0 do |job|
    ...
    for h in handles
      ...
      send_event('twitter_mentions1_56c41d36c44c2834e1d2fbad', comments: twee_arr[0])
      send_event('twitter_mentions2_56c41d36c44c2834e1d2fbad', comments: twee_arr[1])
	  send_event('twitter_mentions3_56c41d36c44c2834e1d2fbad', comments: twee_arr[2])
      send_event('twitter_mentions4_56c41d36c44c2834e1d2fbad', comments: twee_arr[3])
    end
#end
end
...
SCHEDULER.every '40s', :first_in => 0 do |job|
  	...
  	min = 0
  	page_size= 5
    uri = URI('http://0.0.0.0:8080/whooshSearch/?user_id=56c41d36c44c2834e1d2fbad')
    response = Net::HTTP.get(uri)
	...
	if data_n.length>0
        ...
        max = newswhip_article.length
       	...
       	while max > (min+page_size)
            send_event('news_56c41d36c44c2834e1d2fbad', { items: newswhip_article.slice(min, page_size)})
           	sleep(20)
            min = min + page_size
       end
            send_event('news_56c41d36c44c2834e1d2fbad', { items: newswhip_article.slice(min..max)})
    end
end
##----------------------------YOUTUBE-------------------------------------------------------------------------
##-------------------------------------------------------------------------------------------------------
...
if channel.length>0
    ...
    for i in 0..4
       for k in ch.times
         send_event('youtube_player'+(k+1).to_s+'_56c41d36c44c2834e1d2fbad', { :items => channel_arr[k].slice(min, len) })
         k=k+1
       end
       sleep(60)
       min=min+1
