...
def getTopPostsPerSubreddit()  
    ...
    uri = URI('http://0.0.0.0:8080/api/?user_id=56b9954ac44c2823254a026c')
...
SCHEDULER.every '20s', :first_in => 0 do |job|
  ...
  send_event('reddit_56b9954ac44c2823254a026c', { :posts => posts })
end		
...
SCHEDULER.every '20s', :first_in => 0 do |job|
    ...
    for h in handles
      ...
      send_event('twitter_mentions1_56b9954ac44c2823254a026c', comments: twee_arr[0])
      send_event('twitter_mentions2_56b9954ac44c2823254a026c', comments: twee_arr[1])
	  send_event('twitter_mentions3_56b9954ac44c2823254a026c', comments: twee_arr[2])
      send_event('twitter_mentions4_56b9954ac44c2823254a026c', comments: twee_arr[3])
    end
end
  ...
  page_size= 5
    ...
    if tags.length>0
        ...
        while max > (min+page_size)
            send_event('news_56b9954ac44c2823254a026c', { items: newswhip_article.slice(min, page_size)})
            sleep(10)
            min = min + page_size
        end
        send_event('news_56b9954ac44c2823254a026c', { items: newswhip_article.slice(min..max)})
    end
...
arr = []
    ...
    for i in channel_arr[0..5]
        send_event('youtube_player'+(k+1).to_s+'_56b9954ac44c2823254a026c', { :items => i.slice(min, len) })
        k+=1
        sleep(60)
        min=min+1  
    end    
