...
def getTopPostsPerSubreddit()  
    ...
    uri = URI('http://0.0.0.0:8080/api/?user_id=56b86abdc44c280444f3a617')
...
SCHEDULER.every '20s', :first_in => 0 do |job|
  ...
  send_event('reddit_56b86abdc44c280444f3a617', { :posts => posts })
end		
...
#    puts data.length
 if data.length>0
    ...
    for h in handles
      ...
      send_event('twitter_mentions1_56b86abdc44c280444f3a617', comments: twee_arr[0])
      send_event('twitter_mentions2_56b86abdc44c280444f3a617', comments: twee_arr[1])
	  send_event('twitter_mentions3_56b86abdc44c280444f3a617', comments: twee_arr[2])
      send_event('twitter_mentions4_56b86abdc44c280444f3a617', comments: twee_arr[3])
    end
  end
end
   ...
   if data.length>0
    ...
    if tags.length>0
        ...
        while max > (min+page_size)
            send_event('news_56b86abdc44c280444f3a617', { items: newswhip_article.slice(min, page_size)})
            sleep(10)
            min = min + page_size
        end
        send_event('news_56b86abdc44c280444f3a617', { items: newswhip_article.slice(min..max)})
    end
   end   
end
...
arr = []
    ...
    for i in channel_arr[0..5]
        send_event('youtube_player'+(k+1).to_s+'_56b86abdc44c280444f3a617', { :items => i.slice(min, len) })
        k+=1
        sleep(60)
        min=min+1  
    end    
